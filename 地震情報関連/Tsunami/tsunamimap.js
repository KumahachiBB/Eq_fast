...
function jma_tsunamiget(url) {
  ...
  $.when(
    ...
    //2021/08/01追加
    if(Date.parse(tdata[0].Head.ValidDateTime) < new Date()){
      alert("過去の情報です！");
    }
    ...
    if (tdata[0].Body.Earthquake != undefined) {
      ...
      let ep_lon = ep_base.slice(5, 11);
      console.log(ep_lat,ep_lon);
      epicenter_plot = L.marker([parseFloat(ep_lat), parseFloat(-157.9+360)], {
        pane: "eppane",
        icon: epicenter_icon,
      });
